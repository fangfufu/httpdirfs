...
#ifdef DEBUG
#define MEM_HASH_TABLE_SIZE 4096
static MemNode *mem_hash_table[MEM_HASH_TABLE_SIZE] = {0};
static pthread_mutex_t mem_mutex = PTHREAD_MUTEX_INITIALIZER;

static inline size_t hash_ptr(const void *ptr)
{
    return ((uintptr_t)ptr >> 4) & (MEM_HASH_TABLE_SIZE - 1);
}
#endif
...
#ifdef DEBUG
    pthread_mutex_lock(&mem_mutex);
    for (size_t i = 0; i < MEM_HASH_TABLE_SIZE; i++) {
