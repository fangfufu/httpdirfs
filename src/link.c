...
{
    Link **new_links = (Link **)REALLOC((void *)linktbl->links,
                                        (linktbl->size + 1) * sizeof(Link *));
    if (!new_links) {
        lprintf(fatal, "Out of memory in LinkTable_add!\n");
        exit_failure();
    }
    linktbl->links = new_links;
...
LinkTable *LinkTable_disk_open(const char *dirn)
{
    ...
    if (sz < 0 || sz > (int)(INT_MAX / sizeof(Link *))) {
